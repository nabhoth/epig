...
**************************************************************/
void cuBlasMMulti(int columns, qGate *A, qGate *B, qGate *C){
	cublasCgemm ('N','N', columns, columns, columns, make_cuFloatComplex(1, 0),  A->gateMatrix1, columns, B->gateMatrix1, columns, make_cuFloatComplex(0, 0), C->gateMatrix1, columns);
}
/**************************************************************
	...
	char *gate_rep = new char[4];
        ...
        while (end > 0) {
			...
			begin += 3;
				...
				cuBlasMMulti(w, temp0, myfinalG, temp2);
...
//	cout<<"gate 0:  "<<gate_rep<<endl;
			//get the next one
			begin += 3;
			for (int b = begin; b < end; b+=REPSIZE) {
				getGateRepfromStr(b,indi->my_string, gate_rep);
//	cout<<"gate 1:  "<<gate_rep<<endl;
				for (int a = 0; a <= numofgates; a++)
					if (compareRep(indi->gateArray[a]->representation, gate_rep)) {
						temp1 = ccGate(gateArray[a], temp1);
						temphase = indi->phases[phasecounter0++];
			...
			if (Time == 0) {
				...
				/* Initialize the device matrices with the host matrices */
				(cudaMemcpy(d_M1, myfinalG->gateMatrix1, N*sizeof(cuComplex), cudaMemcpyHostToDevice));
				Time++;
			} else {
				if (even_odd == 0){
					if (Time == 1){
