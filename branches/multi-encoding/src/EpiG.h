...
// for a particular number of input and output, the list of the gates can be used
	char *nameGate[MAXNUMOFGATES+10];	
};
...
struct result
{
	float avFitness [MAXGEN];
	float avError[MAXGEN];
	float avCost[MAXGEN];
	int counter;
	int step;
};
...
//each circuit is now generated with phase
	cuComplex phase;	
	cuComplex phases[MAXNUMOFGATES+10];
// fitness value
	float fitness;		
// Cost of the circuit
	float Cost;		
...
// all agtes are stored in here
	qGate *gateArray[MAXNUMOFGATES+10];	
	int valuedness;
...
// compare two gates
void printqGate(qGate*);
int getGatesNumber(string);
bool compareGate (qGate*, qGate*);
bool compareRep (char*, char*);
bool isIn(int, int[], int);
int isInAt(int, int[], int);
void copyRep (char*, char*);
void copyRepUnchecked (string, char*, int);
void incRep(char*);
void incRep(char*, int, int);
void initRep(char*, int);
void initRep(char*);
int getSegmentNumber(string);
int getGatePos(int, string);
int getGateRepfromStr(int, string, char*);
bool checkRestrictions(int, qGate*);
...
bool matrixCompare(int, int, cuComplex*, cuComplex*);
void segAnaAndMerge(char*);
...
#ifdef __CUDA__
void cuBlasMMultiSet(int, cuComplex*, cuComplex*, cuComplex*, cuComplex*, cuComplex*, cuComplex*);
void cuBlasMMulti(int, cuComplex*, cuComplex*, cuComplex*);
...
__global__ void matrixMul( cuComplex*, cuComplex*, int, int, cuComplex*);
__global__ void matrixKron(cuComplex*, cuComplex*, int, int, cuComplex*);
...
__global__ void initNewState(int, cuComplex, cuComplex*, int);
...
void Kronm(cuComplex*, cuComplex*, int, int, cuComplex*, cuComplex*, cuComplex*, cuComplex*);
void Kronm(qGate*, qGate*, int, int, qGate*, cuComplex*, cuComplex*, cuComplex*);
//matrix multiplication using device matrices pointers
void Mulmnoset(int, cuComplex*, cuComplex*, cuComplex*); 
void Kronmnoset(int, int, cuComplex*, cuComplex*, cuComplex*);
//matrix multiplication using device matrices pointers
...
//array of gates to be used only for specified gates
...
// Replication of teh selected individuals either SUS or RW
	...
	void makeFitness(Individual*);


	void applyRMutation (int);
//A mutation operator that does preserbe the current circuit structure completely
	void applyRMutationP(int);		
	void applyIMutation (int);
//A mutation operator that does preserbe the current circuit structure completely
	void applyIMutationP(int);		
...
// method minimizing circuit ... not used always
	...
	void restructureCircuit (Individual *, bool);
	void restructureCircuitD (Individual *);
...
//the set of probabilistic expectations generated as the result of measurement of the individual states
	cuComplex *expectationsAllState[MAXGATEINPUT*MAXVALUEDNESS];   
...
// looks for the best individual with fitness 1
	bool getFitness ();	
	nameT *normNames[MAXNUMOFGATES+10];
...
//two fields per input by Max size of the input-output mapping
	cuComplex *measureexpected[1024]; 
	int measurementQBits[MAXGATEINPUT];
	int measured_desired_output_records;
	int *measured_desired_output_records_idx;
	int *non_measured_desired_output_records_idx;
	qGate *permutativeArray[MAXNUMOFGATES+10];
// all agtes are stored in here
	qGate *gateArray[MAXNUMOFGATES+10];
//list used to rank all gates based on their usage and efficiency
	vector<rGate> rankV;
...
//////////////////////////////////////////////////////////
	...
	int mincircuitsize;
	//multiplier to allow larger circuits to exists
	int segments_multi;
	...
	int generations;
	//switch allowing to push all circuits away from the Identiy local minimum
	int local_forcing;
	//switch allowing to force exact match up to the phase 
	int force_exact;
...
//the array holding the resulting desired sequence
	...
	//cuComplex *sequence_desired[MAXSEQUENCESIZE];	
...
// merge two equivalent or equal segments
	...
	int getTopWire(int, string);
	void parseCircuitIntoArray(string, int, int**);
	int compareIntSegments(int*, int*, char**, char**, int**, int**, int);
	int analyzeSegment(char**, int, bool);
	void rankInputGates(Individual*);
	void sortGates();
	int getRandomGate(int, bool, int, int);
	int getBestGateMatch(float, int, int, int, bool);
};
