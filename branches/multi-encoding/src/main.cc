...
int main(int argc, char* argv[])
{
	...
	cout<<"\nOutput path: "<<out_path<<" in file: "<<str<<endl;
	//ignition
	GA g;
	//QE q;
	srand( time(NULL) );
	...
	time = clock() ;
#endif
		datadisplaycounter++;
		if (generationCondition % 50 == 0)
			cout<<" current generation is "<<generationCondition<<endl;
		cout<<" replication starting"<<endl;
		g.applyReplication();
		cout<<" replication done, starting mutation"<<endl;
		g.applyMutation();
		cout<<" mutation done, optimization start"<<endl;
		g.applyOptimization(true);
		cout<<" optimization done, fitness start"<<endl;
		bestcounter = 0;
		...
		cout<<"fitness end, evaluation start"<<endl;
		g.evaluatePopulation();
