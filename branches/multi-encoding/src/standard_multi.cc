 ...
 * returns the matrix representing the circuit of a given individual
 ****************************************/
qGate* GA::computeStringMatrix(string s, int valuedness, int io) {
	...
	int N = (int(pow(pow((float) valuedness, (float) io),2)));
	cuComplex alpha = make_cuFloatComplex(1,0);
	cuComplex beta = make_cuFloatComplex(0,0);

	initGate(temp0, io, valuedness, 1);	
	initGate(temp1, io, valuedness, 1);	
	initGate(temp2, io, valuedness, 1);	
	initGate(myfinalG, io, valuedness, 1);	
	//int the string counters
	int begin = 0;
	...
	while (end > 0) {
		if ((end - begin) > 1) {
			...
			begin = s.find("p", end);
			if (end != -1 && s.at(end) == 'p' && end < s.length() - 3)
				end = end - 1;
		} else {
		//move to the next segment
			begin = end+1;
