...
//	out_stream << "Individual matrix being generated for "<<indi->my_string<<endl;
...
/*out_stream<<"first gate"<<endl;
		maxcount = int((pow(val, temp0->numIO)));
		for (int a = 0; a < maxcount; a++) {
		for (int b = 0; b < maxcount; b++) {
				out_stream << "(" << cuCrealf(temp0->gateMatrix1[a*int(maxcount)+b])<< "," << cuCimagf(temp0->gateMatrix1[a*int(maxcount)+b]) << ")"<< "";
...
/*out_stream<<"next gate"<<endl;
		maxcount = int((pow(val, numIO)));
		for (int aa = 0; aa < maxcount; aa++) {
		for (int bb = 0; bb < maxcount; bb++) {
				out_stream << "(" << cuCrealf(temp1->gateMatrix1[aa*int(maxcount)+bb])<< "," << cuCimagf(temp1->gateMatrix1[aa*int(maxcount)+bb]) << ")"<< "";
...
/*out_stream<<"result gate"<<endl;
		maxcount = int((pow((float)val, (float)temp0->numIO)));
		for (int aa = 0; aa < maxcount; aa++) {
		for (int bb = 0; bb < maxcount; bb++) {
				out_stream << "(" << cuCrealf(temp0->gateMatrix1[aa*int(maxcount)+bb])<< "," << cuCimagf(temp0->gateMatrix1[aa*int(maxcount)+bb]) << ")"<< "";
...
/*out_stream<<"first"<<endl;
		for (int a = 0; a < w; a++) {
		for (int b = 0; b < w; b++) {
				out_stream << "(" << cuCrealf(temp0->gateMatrix1[a*w+b])<< "," << cuCimagf(temp0->gateMatrix1[a*w+b]) << ")"<< "";
...
/*out_stream<<"second"<<endl;
		for (int a = 0; a < w; a++) {
		for (int b = 0; b < w; b++) {
				out_stream << "(" << cuCrealf(temp2->gateMatrix1[a*w+b])<< "," << cuCimagf(temp2->gateMatrix1[a*w+b]) << ")"<< "";
...
qGate* GA::computeMatrix(Individual *indi) {
	...
	initGate(temp2, indi->ioNumber, indi->valuedness, 1);	
	initGate(myfinalG, indi->ioNumber, indi->valuedness, 1);	
//	cout << "Individual matrix being generated for0 "<<indi->my_string<<endl;
	//int the string counters
	int begin = 0;
	int end = indi->my_string.find("p", begin + 1);
	indi->Cost = 0;
	...
	//cout << "Individual matrix being generated for "<<indi->my_string<<endl;
	while (end > 0) {
		if ((end - begin) > 1) {
			csegs++;
			//get the first  gate of the Segment and copy it
...
void* GA::doMultiMeasureFitness(Individual *ind, bool display) {
	...
	if (indi->Error != 0) {
		...
		case 3:
			//scaled complex fitness2
			indi->fitness = (alpha * (1 / (indi->Error + 1)) + beta
					* indi->Cost);
			break;
		case 4:
			indi->fitness = (1 - indi->Error);
